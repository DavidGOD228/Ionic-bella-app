...
≈                            <div style={{ color: 'white', paddingLeft: 20 }}>{'Выберите Дату'}</div>
      ...
      {isShowModal &&
                <div className="month-modal">
                  {months.map((month, i) => <span key={month} onClick={() => setSelectedMonth(moment().add(i, 'month').format())} style={{ display: 'block', padding: 15, backgroundColor: moment(month, 'MMMM').format('MMM') ===  moment(selectedMonth).format('MMM') ? '#F5F5F7' : '#fff' }}>{month}</span>)}
                </div>
      }
      <div className="wrapper">
        <DayPicker captionElement={({ date, localeUtils }) => <div></div>
      ...
      <div className="dates-container-wrapper">
        ...
        <div className="dates-container">
          {store.dates.map((el) => <div key={el.title}>
            <span style={{ marginLeft: 10, color: '#A7A7AB' }}>{el.title}</span>
            <div className="dates-wrapper">
              {el.dates.map((date) => <div key={date} onClick={() => onSelectTime(date)} className={'date-item'}>{date}</div>)}
            </div>
          </div>)}
        </div>
      </div>
