    ...
    <IonPage>
      ...
      <IonContent fullscreen onClick={() => setShowAboutInfo(false)}>
        ...
        <div className={'city-container flex-row'}>
          {cityList && cityList.map((item) => <div className={`${currentCity === item ? 'select-item-active' : ''} select-item`}
            onClick={() => setCurrentCity(item)}>
            {item}
          </div>)}
        </div>
        {isList && <div className={'affiliates-container'}>
          {affiliates && affiliates.map(affiliate => <AffiliateItem affiliate={affiliate}/>)}
        </div>}
      </IonContent>
